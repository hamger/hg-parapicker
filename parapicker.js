...
(function(global, factory) {
    ...
    ParaPicker.prototype = {
        ...
        bindRoll: function(i) {
            var that = this
            that.paraUl[i].addEventListener('touchstart', function() {
                that.touch(i)
            }, false)
            that.paraUl[i].addEventListener('touchmove', function() {
                that.touch(i)
            }, false)
            that.paraUl[i].addEventListener('touchend', function() {
                that.touch(i)
            }, true)
        },
        /**
        ...
        getPosition: function(i) {
            if (this.curDis[i] <= -1 * (this.liNum[i] - 1) * this.liHeight ) this.paraIndex[i] = this.liNum[i] - 1
